class SizesController < ApplicationController
  ...
  def index
    @sizes = Size.with_deleted.all.page(params[:page]).per(5)
  end
  ...
  def restore
    Size.restore(params[:id])
    respond_to do |format|
      format.html { redirect_to sizes_path, status: :see_other, notice: "Size was successfully restored." }
      format.json { head :no_content }
    end
  end
