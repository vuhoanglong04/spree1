class ColorsController < ApplicationController
  ...
  def index
    @colors = Color.with_deleted.all.page(params[:page]).per(5)
  end
  ...
  def restore
    Color.restore(params[:id])
    respond_to do |format|
      format.html { redirect_to colors_path, status: :see_other, notice: "Color was successfully restored." }
      format.json { head :no_content }
    end
  end
